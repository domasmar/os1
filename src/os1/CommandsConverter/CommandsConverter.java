...
public class CommandsConverter {
	...
	/* Iš paduoto pirminio kodo teksto pašalina visas tuščias eilutes ir tarpus bei į masyvą "sourceCode" įrašo „švaraus“ kodo eilutės. */
	...
	/* Į sąrašą išsaugo tik tas pirminio kodo eilutes, kuriose yra komandos. Grąžina komandų masyvą. */
	...
	private void validateSourceCode() throws Exception {
		...
		for (int i = 0; i <= this.sourceCode.length - 1; i++) {
			...
			/* Tikrinamos kodo eilutės, sudarytos iš vieno žodžio (ADD, SUB, STOP komandos, labelių pasižymėjimas. */
			else {
				isCommand = false;
				for (int j = 0; j <= machineCommands.length - 1; j++) {
